...
export class CharacterMaterial extends MeshStandardMaterial {
  ...
  constructor(private config: CharacterMaterialConfig & { isWebGPU?: boolean }) {
    super();
    this.copy(this.config.originalMaterial);
    this.generateColorCube();
    ...
    // 🔑 WebGPU path: use alphaHash instead of onBeforeCompile hacking
    if (this.config.isWebGPU) {
      this.transparent = false;        // alphaHash expects opaque material
      (this as any).alphaHash = true;  // built-in hashed transparency
      // keep using this.opacity driven by update() – alphaHash respects it
    } else {
      // WebGL path: keep your existing onBeforeCompile hook
      this.transparent = true;
      ...
      this.onBeforeCompile = (shader) => {
        ...
        // Guard against duplicate declarations of vUv on some materials
        const uvDecl = `
#ifndef __HAS_VUV
#define __HAS_VUV
varying vec2 vUv;
...
if (discardAll == 1) {
  ...
  ivec2 p = ivec2(mod(gl_FragCoord.xy, 8.0));
  float d = 0.0;
  if (p.x <= 3 && p.y <= 3) { d = bayerDither(bayertl, p); }
  else if (p.x > 3 && p.y <= 3) { d = bayerDither(bayertr, p - ivec2(4, 0)); }
  else if (p.x <= 3 && p.y > 3) { d = bayerDither(bayerbl, p - ivec2(0, 4)); }
  else if (p.x > 3 && p.y > 3) { d = bayerDither(bayerbr, p - ivec2(4, 4)); }
  if (distance <= ditheringNear + d * ditheringRange) discard;
  outgoingLight *= diffuseRandomColor;
}
#include <dithering_fragment>
