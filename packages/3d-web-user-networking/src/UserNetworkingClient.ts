import {
	...
	type DeltaNetClientWebsocketInitialCheckout,
	DeltaNetClientWebsocketStatus,
	type DeltaNetClientWebsocketTick,
	type DeltaNetClientWebsocketUserIndex,
...
import {
	...
	type UserNetworkingLogger,
} from "./UserNetworkingLogger";
import type { CharacterDescription } from "./UserNetworkingMessages";
...
export class UserNetworkingClient {
	...
	constructor(
		...
		this.deltaNetClient = new DeltaNetClientWebsocket(
			...
			config.sessionToken,
			{
				ignoreData: false,
				onInitialCheckout: (
					initialCheckout: DeltaNetClientWebsocketInitialCheckout,
				) => {
	...
	private sendInitialAuthentication(): void {
		...
		const components = DeltaNetComponentMapping.toComponents(
			this.pendingUpdate,
		);
	...
	private processNetworkUpdate(
		...
		for (const stableId of addedStableIdsArray) {
			...
			const userIdState = stableUserData.states.get(
				STATE_INTERNAL_CONNECTION_ID,
			);
			if (!userIdState) {
			...
			if (!userId) {
				throw new Error(
					`Failed to extract userId from bytes for stableId ${stableId}`,
				);
			}
			this.stableIdToUserId.set(stableId, userId);
			const newProfile = DeltaNetComponentMapping.fromStates(
				stableUserData.states,
				this.logger,
			);
			this.userProfiles.set(userId, newProfile);
			const clientUpdate = DeltaNetComponentMapping.fromComponents(
				stableUserData.components,
			);
		...
		for (const update of stateUpdates) {
			...
			if (!profile) {
				this.logger.warn(
					`No profile found for user ${userId}, skipping update`,
				);
				continue;
			}
			const existingUpdate = updatedUsers.get(userId)!;
			let existingUserStateUpdate: Partial<UserData> | undefined =
				existingUpdate.userState;
			...
			switch (update.stateId) {
				case STATE_INTERNAL_CONNECTION_ID:
					...
					break;
				case STATE_USERNAME: {
					const username = DeltaNetComponentMapping.usernameFromBytes(
						update.state,
					);
					if (username) {
				...
				case STATE_CHARACTER_DESCRIPTION: {
					const characterDescription =
						DeltaNetComponentMapping.characterDescriptionFromBytes(
							update.state,
						);
				...
				case STATE_COLORS: {
					const colors = DeltaNetComponentMapping.decodeColors(
						update.state,
						this.logger,
					);
	...
	public updateCharacterDescription(
		...
		// Send state update
		const states =
			DeltaNetComponentMapping.toCharacterDescriptionState(
